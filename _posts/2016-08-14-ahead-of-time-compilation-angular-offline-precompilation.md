...
The short answer of this question is - **We need compilation for achieving higher level of efficiency of our Angular applications.** By efficiency I mostly mean performance but also energy and bandwidth consumption.
...
We will go quickly through the process of compilation because there's no point to explain the entire `@angular/compiler` line-by-line. If you're interested in the process of lexing, parsing and code generation, you can take a look at the talk ["The Angular 2 Compiler" by Tobias Bosch](https://www.youtube.com/watch?v=kW9cJsvcsGo) or this [slide deck](https://speakerdeck.com/mgechev/angular-toolset-support?slide=69).
...
`*` is a placeholder for the file's name, so `hero.ts` will produce: `hero.ngfactory.ts`, `hero.css.shim.ts`, and `hero.metadata.json`.

`*.css.shim.ts` is not very interesting for the purpose of our discussion so we won't describe it in details. If you're interested in more details for `*.metadata.json` you can take a look at the section: "AoT and third-party modules".
...
Let's suppose that `currVal_6` has value `3` and `this_expr_6` has value `1`; and now lets trace the method execution:
...
In the example above `this.context` will be equal to `new HeroComponent()`. This means that in `detectChangesInternal` we have to access `this.context.name`. Here comes a problem. **If we want to emit TypeScript as output of the AoT compilation process we must make sure we've bound only to public properties.** Why is that? As we already mentioned the compiler can generate both TypeScript and JavaScript. Since TypeScript has access modifiers and enforces access only to public properties outside the inheritance chain inside the internal component we cannot access any private properties part of the context object, so both:
...
will throw compile time errors, since in the first case the internal component won't be able to access `hero` since it's private within `HeroComponent`, and in the second case the internal component won't be able to access `hero.name`, since `name` is private inside `Hero`.
...
This works perfect - each time the value of the `name` expression changes and equals to truthy value, the `_initials` property will be recomputed. However, the implementation of `ThirdPartyComponent` is not AoT-friendly. If we want to do so we have to change it to:
...
```
...which will leave the `ThirdPartyComponent` component in inconsistent state. The since the value of the `_name` property of the `ThirdPartyComponent`'s instance will be `Foo Bar` but the value of `initials` will be `M. D.`.

The answer of how to solve this issue is rooted in the Angular's code. In case we want to make our code AoT-friendly (i.e. bind only to public properties and methods in templates), and in the same time keep encapsulation, we can use the TypeScript annotation `/** @internal */`:
...
The `initials` property will be public but if we compile our third-party library with `tsc` and the `--stripInternal` flag set, the `initials` property will be omitted from the type definition file for `ThirdPartyComponent`.
