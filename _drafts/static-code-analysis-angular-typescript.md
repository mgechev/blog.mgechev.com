...
- Gather metadata for component
  ...
  - Based on the AST we create an intermediate Angular specific component representation
    ...
    - We collect all `directives` (note that the directives here are actually references to other classes)
      ...
      - Add the directive with its entire subtree (if component) to the directives array of the component
...
- Validate given component
- Use the parser in order to get the AST of the component's template (we already have validation for pipes and directives which are not found yet thanks to the parser of Angular which will throw an error in case it doesn't find them)
...
The problem now is that although we've parsed both the source file and the template, we haven't parsed the Angular expression. Yes, Angular defines a small DSL which allows us to execute different expressions in the context of given component.
